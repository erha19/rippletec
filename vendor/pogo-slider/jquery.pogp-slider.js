...
(function ( $, window, document, undefined ) {
	...
	Plugin.prototype = {
		...
		_createDirButtons: function () {
			...
			$('<button class="pogoSlider-dir-btn pogoSlider-dir-btn--prev" style="z-index:999"></button>').appendTo(self.$element).on('click', function () {
				self.prevSlide();
			});

			$('<button class="pogoSlider-dir-btn pogoSlider-dir-btn--next" style="z-index:999"></button>').appendTo(self.$element).on('click', function () {
				self.nextSlide();
			});
		...
		_createNavigation: function () {
			...
			var $navContainer = $('<ul class="pogoSlider-nav" style="z-index:999"></ul>').appendTo(self.$element);
		...
		_changeSlide: function (prevSlideIndex,currentSlideIndex) {
			...
			setTimeout(function () {
				...
				self.slides[currentSlideIndex].$element.css('z-index', 20);
		...
		slideTransitions:  {
			...
			slideLeft: function (prevSlideIndex,currentSlideIndex) {
				...
				return function () {
					self.slides[prevSlideIndex].$element.removeClass('pogoSlider-animation-leftOut').css({'z-index':'1'});
					currentSlide.$element.attr('style',currentSlide.$element.data('original-styles')).css({'opacity':'1','z-index':'10'}).removeClass('pogoSlider-animation-leftIn');
				};
			...
			slideRight: function (prevSlideIndex,currentSlideIndex) {
				...
				return function () {
					...
					currentSlide.$element.attr('style',currentSlide.$element.data('original-styles')).css({'opacity':'1','z-index':'10'}).removeClass('pogoSlider-animation-rightIn');
				};
